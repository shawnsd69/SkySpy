...
# Video path
video_path = r"E:\ANPR_YOLOv10\Resources\carLicence8.mp4"
...
ocr_results = {}  # Format: {vid: [{'plate': str, 'confidence': float, 'frame': int}, ...]}
...
def preprocess_plate_image(frame, x1, y1, x2, y2):
    ...
    try:
        ...
        scale_factor = 2
        resized = cv2.resize(sharpened, None, fx=scale_factor, fy=scale_factor, interpolation=cv2.INTER_LINEAR)
...
def correct_plate_text(text):
    """Correct common OCR errors in the license plate text."""
    text = text.upper()
    # Specific corrections for MH02CW3902
    if len(text) >= 6:
        
        series = text[4:6]
        if series in ['CV', 'CH']:
            text = text[:4] + 'CW' + text[6:]
        elif series[0] in ['V', 'H']:
            text = text[:4] + 'C' + text[5:]
        elif series[1] in ['V', 'H']:
            text = text[:5] + 'W' + text[6:]
    return text

def paddle_ocr(frame, x1, y1, x2, y2):
    try:
        # Preprocess the plate image
        plate_img = preprocess_plate_image(frame, x1, y1, x2, y2)
        ...
        confidence = 0
        for r in result:
            if r and len(r) > 0 and len(r[0]) > 1:
                scores = r[0][1]
                if np.isnan(scores):
                    scores = 0
                else:
                    scores = float(scores)
                if scores > 0.6:  # Adjusted confidence threshold
                    text = r[0][0]
                    confidence = scores
...
def select_best_plate(ocr_list):
    """Select the most likely plate number based on frequency and confidence."""
    if not ocr_list:
        return None
    
    # Count frequency of each plate
    plate_counts = {}
    for entry in ocr_list:
        plate = entry['plate']
        if plate:
            if plate not in plate_counts:
                plate_counts[plate] = {'count': 0, 'total_confidence': 0.0}
            plate_counts[plate]['count'] += 1
            plate_counts[plate]['total_confidence'] += entry['confidence']
    
    # Calculate average confidence and select the best plate
    best_plate = None
    best_score = 0
    for plate, data in plate_counts.items():
        avg_confidence = data['total_confidence'] / data['count']
        score = data['count'] * avg_confidence
        # Boost score for plates matching 'MH02CW....' pattern
        if plate.startswith('MH02CW'):
            score *= 1.5  # Prioritize 'CW' series
        if score > best_score:
            best_score = score
            best_plate = plate
            ...
            if vdata['stationary'] and vdata['class_name'] == 'car' and not vdata['plate']:
                ...
                for result in plate_results:
                    for box in result.boxes:
                        ...
                        label, confidence = paddle_ocr(best_frame, px1, py1, px2, py2)
                        if label:
                            # Store OCR result
                            if vid not in ocr_results:
                                ocr_results[vid] = []
                            ocr_results[vid].append({
                                'plate': label,
                                'confidence': confidence,
                                'frame': frame_count
                            })
                            print(f"OCR result for ID {vid}: {label} (confidence: {confidence:.2f})")
        
                # Select the best plate if enough results are collected
                if vid in ocr_results and len(ocr_results[vid]) >= 3:  # Wait for at least 3 OCR results
                    best_plate = select_best_plate(ocr_results[vid])
                    if best_plate:
                        vdata['plate'] = best_plate
                        license_plates.add(best_plate)
                        print(f"Selected best plate for ID {vid}: {best_plate}")
                        # Save the entire frame locally
                        frame_path = os.path.join(OUTPUT_DIR, f"frame_{frame_count}_vehicle_{vid}_plate_{best_plate}.jpg")
                        ...
                        image_url = upload_image(display_frame, timestamp, vid, best_plate)
                        if image_url:
                            log_violation(best_plate, image_url, timestamp)
                        # Clear OCR results for this vehicle
                        del ocr_results[vid]
